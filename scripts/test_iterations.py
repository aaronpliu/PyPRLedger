...
review1 = create_review(
    "pr-test-iter", COMMIT_ID, "diana_reviewer", "First review - iteration 1", 7
)
print(
    f"   ID: {review1.get('id')}, Iteration: {review1.get('review_iteration')}, Is Latest: {review1.get('is_latest_review')}"
)

print("\n📝 Creating Review 2 (Iteration 2) - Same Commit")
review2 = create_review(
    "pr-test-iter", COMMIT_ID, "diana_reviewer", "Second review - iteration 2", 8
)
print(
    f"   ID: {review2.get('id')}, Iteration: {review2.get('review_iteration')}, Is Latest: {review2.get('is_latest_review')}"
)

print("\n📝 Creating Review 3 (Iteration 3) - Same Commit")
review3 = create_review(
    "pr-test-iter", COMMIT_ID, "diana_reviewer", "Third review - iteration 3", 9
)
print(
    f"   ID: {review3.get('id')}, Iteration: {review3.get('review_iteration')}, Is Latest: {review3.get('is_latest_review')}"
)
...
for item in data.get("items", []):
    print(
        f"   - ID: {item['id']}, Reviewer: {item['reviewer']}, Iteration: {item['review_iteration']}, Is Latest: {item['is_latest_review']}"
    )
