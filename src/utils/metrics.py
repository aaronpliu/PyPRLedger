...
class MetricsCollector:
    ...
    def _init_metrics(self):
        ...
        self.active_reviewers = Gauge(
            "active_reviewers", "Number of active reviewers", ["project"], registry=self.registry
        )
        ...
        self.reviewers_total = Gauge(
            "reviewers_total", "Total number of reviewers", registry=self.registry
        )

        self.reviewers_active = Gauge(
            "reviewers_active", "Number of active reviewers", registry=self.registry
        )

        # Project metrics
        self.projects_total = Gauge(
            "projects_total", "Total number of projects", registry=self.registry
        )

        self.projects_active = Gauge(
            "projects_active", "Number of active projects", registry=self.registry
        )

        # Repository metrics
        self.repositories_total = Gauge(
            "repositories_total", "Total number of repositories", registry=self.registry
        )
        ...
        self.cache_hits_total = Counter(
            "cache_hits_total", "Total number of cache hits", ["cache_type"], registry=self.registry
        )
        ...
        self.system_memory_available_bytes = Gauge(
            "system_memory_available_bytes", "Available memory in bytes", registry=self.registry
        )

        self.system_disk_usage_bytes = Gauge(
            "system_disk_usage_bytes", "Disk usage in bytes", ["path"], registry=self.registry
        )
        ...
        self.errors_total = Counter(
            "errors_total", "Total number of errors", ["type", "endpoint"], registry=self.registry
        )
        ...
        if self.start_time:
            ...
            if self.operation_type == "http_request":
                ...
                self.metrics.observe_review_duration(
                    project=self.labels.get("project", "unknown"), duration=duration
                )
