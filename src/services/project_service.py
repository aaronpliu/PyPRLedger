...
class ProjectService:
    ...
    async def get_project_with_stats(
        ...
        repo_query = select(func.count()).select_from(Repository).where(
        ...
        # Get review count
        review_query = select(func.count()).select_from(PullRequestReview).where(
            PullRequestReview.project_id == project_id
        )
        review_result = await db.execute(review_query)
    ...
    async def get_project_repository_count(
        ...
        # Query database
        query = select(func.count()).select_from(Repository).where(
            Repository.project_id == project_id
        )
        result = await db.execute(query)
        count = result.scalar()
    ...
    async def get_project_review_count(
        ...
        # Query database
        query = select(func.count()).select_from(PullRequestReview).where(
            PullRequestReview.project_id == project_id
        )
        result = await db.execute(query)
        count = result.scalar()
    ...
    async def get_projects_with_most_reviews(
        ...
        date_threshold = date_threshold.replace(day=date_threshold.day - days) if date_threshold.day > days else date_threshold.replace(day=1)
    ...
    async def get_projects_with_most_active_reviewers(
        ...
        query = (
            select(
                ...
                Project.project_id,
                Project.project_name,
                func.count(func.distinct(PullRequestReview.reviewer_id)).label('active_reviewer_count')
            )
            .join(PullRequestReview, Project.id == PullRequestReview.project_id)
            .where(PullRequestReview.created_date >= date_threshold)
            .group_by(Project.id)
